...
## 17. Variation kcal deltas can be implausibly low (ROADMAP ITEM, found live 2026-07-22)

**Motivating case:** "Asian-Style Mango Sticky Rice" (base 350 kcal / 2 servings = 175 kcal/person) has a "With Condensed Milk" variation adding 50ml condensed milk, but its override `kcal` is only 400 total — a 50 kcal delta (25/person), when 50ml of real condensed milk is closer to ~150 kcal on its own. The model computed the variation's `kcal` override implausibly low for what it actually added.

**Why the existing audit didn't catch it:** `recipe_audit.py`'s `_kcal_findings`/`evaluate_kcal_plausibility` only reads the recipe's own top-level `ingredients`/`kcal` columns — it has no check at all for the `kcal` value living inside each entry of the `variations` jsonb array. A variation's kcal override is generated at the same time as the base recipe (same `submit_recipe` tool call, see `tools.py`'s `variations` schema) but never independently sanity-checked against its own `add` ingredients the way the base recipe's kcal is checked against its own ingredient list.

**When this is picked up:** extend `evaluate_kcal_plausibility` (or a sibling check) to also validate each variation's `kcal` against `base_kcal + rough_estimate(variation.add ingredients)` — same rough kcal-per-gram heuristic already used for the base check, just applied to the delta instead of the total. Flag-and-report only, same tier as the existing kcal checks (heuristic, not semantic/blocking).
